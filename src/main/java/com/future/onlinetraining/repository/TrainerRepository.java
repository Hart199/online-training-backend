...
public interface TrainerRepository extends JpaRepository<User, Integer> {
    ...
    @Query (
            ...
            value = "select u.id, u.fullname as name, u.photo, avg(tr.value) as rating " +
                    "from users u inner join trainer_ratings tr " +
                    "on u.id = tr.trainer_id inner join roles r " +
                    "on r.id = u.role_id where r.value = 'TRAINER' " +
                    "group by u.id order by rating desc"
    )
